...
function generateLlmsTxt () {
  const parts = [
    ...
    '- Queries that select individual shots are watched as clips, so',
    '  frame each result: add CONTEXT BEFORE 1 shot and CONTEXT AFTER 1',
    '  shot by default (a lead-in and lead-out). Drop the side that',
    '  cannot exist — no BEFORE for a serve (the rally\'s first shot), no',
    '  AFTER for a rally-ending shot (a winner or the final fault) — and',
    '  drop both for SELECT / aggregate / GROUP BY queries and plain',
    '  counts (CONTEXT is not allowed with GROUP BY). Widen when asked:',
    '  CONTEXT BEFORE rally for the whole point, or a secs/min/max',
    '  duration. CONTEXT never changes which shots match: to SELECT the',
    '  shot before/after an event, put a shot[1]/shot[-1] condition in',
    '  WHERE (the shot right before a winner: WHERE',
    '  exists(shot[1].winnerType)).',
