...
function generateLlmsTxt ({ withCli = true } = {}) {
  const parts = [
    ...
    '- There is no UNION, no subquery and no FROM-less SELECT. Two ways to',
    '  chart a comparison, and both beat hand-writing the numbers: when the',
    '  categories are values of one property, GROUP BY it (GROUP BY',
    '  shot.errors.unforced returns a row each for true, false and unknown);',
    '  when they are unrelated conditions, put each in its own aggregate',
    '  column (SELECT sum(shot.errors.unforced) AS "My errors",',
    '  sum(shot.winnerType = "forced_fault") AS "Faults I forced").',
    '  Never SELECT a literal you worked out yourself: a chart is supposed',
    '  to show what the query found.',
