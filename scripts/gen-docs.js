...
function generateLlmsTxt () {
  const parts = [
    ...
    '    [CONTEXT BEFORE 1 shot]         -- lead-in; include by default for shot clips',
    '    [CONTEXT AFTER 1 shot]          -- lead-out; include by default for shot clips',
    ...
    '  GROUP BY) is watched as clips, so ALWAYS frame each result with a',
    '  one-shot lead-in and lead-out — add BOTH `CONTEXT BEFORE 1 shot` and',
    '  `CONTEXT AFTER 1 shot`. Drop only the side that cannot exist: no',
    '  BEFORE for a serve (the rally\'s first shot), no AFTER for a',
    '  rally-ending shot (a winner or the final fault). Use exactly',
    '  `1 shot` on each side — not `rally`, not `2secs` — unless the',
    '  question explicitly asks for more footage (then `CONTEXT BEFORE',
    '  rally` for the whole point, or a secs/min/max duration). Omit CONTEXT',
    '  entirely ONLY for SELECT / aggregate / GROUP BY / count queries',
    '  (CONTEXT is not allowed with GROUP BY). CONTEXT never changes which',
    '  shots match: to SELECT the shot before/after an event, put a',
    '  shot[1]/shot[-1] condition in WHERE (the shot right before a winner:',
    '  WHERE exists(shot[1].winnerType)).',
